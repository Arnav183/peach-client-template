...
export function getActiveServices(ids: string[]): Service[] {
  const set = new Set([...ids, 'crm_dashboard']);
  return ALL_SERVICES.filter(s => set.has(s.id));
}

export function serviceIsActive(id: string, ids: string[]): boolean {
  return id === 'crm_dashboard' || ids.includes(id);
}
